...
enum ShamanSpells
{
    ...
    //For Earthen Power
    SPELL_SHAMAN_STORM_EARTHS_GRASP           	= 51483,
...
public:
    ...
    class spell_sha_mana_tide_AuraScript : public AuraScript
    {
        ...
        {
            if (!sSpellStore.LookupEntry(SHAMAN_SPELL_MANA_TIDE))
                    return false;
                return true;
        }
        ...
        void Register()
        {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_sha_mana_tide_AuraScript::HandleEffectCalcAmount, EFFECT_0, SPELL_AURA_MOD_STAT);
        }
    };
...
class spell_sha_earthbind_totem : public SpellScriptLoader
{
    public:
        ...
        class spell_sha_earthbind_totem_AuraScript : public AuraScript
        {
            ...
            bool Validate(SpellEntry const* /*spellInfo*/)
            {
                if (!sSpellStore.LookupEntry(SPELL_SHAMAN_TOTEM_EARTHBIND_TOTEM) || !sSpellStore.LookupEntry(SPELL_SHAMAN_TOTEM_EARTHEN_POWER))
                    return false;
                return true;
            }
            ...
            void Apply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                ...
                if (!owner)
                    return;
                // Talent Earth's Grasp
				if((owner->HasAura(SPELL_SHAMAN_STORM_EARTHS_GRASP)) || (owner->HasAura(51485))){
                AuraEffect* aurEff = owner->GetAuraEffectOfRankedSpell(SPELL_SHAMAN_STORM_EARTHS_GRASP, EFFECT_1);
                    if (roll_chance_i(aurEff->GetAmount()))
