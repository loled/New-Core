...
public:
    ...
    {
        // Now we will check if the player is in combat. 
	if(player->isInCombat())
	{
		// Telling if the player can't open/doesn`t have acces to the option menu
		player->GetSession()->SendAreaTriggerMessage("|cffff0000You are in combat already!");
		return false; 
	}
	...
	void addMainMenu(Player* player, Creature* creature){
		if (player->isInCombat())
        {
            player->CLOSE_GOSSIP_MENU(); // Now lets close the menu if the player in combat already
            creature->MonsterWhisper("|cffff0000You are in combat!", player->GetGUID());
        }
        else
        {
        ...
        if (sender == 1)
        {
			...
			case 10: // restore mana & hp
				...
				player->SetHealth(player->GetMaxHealth());
				 _creature->MonsterWhisper("We are done restoring your health & mana!", player->GetGUID());
				 addMainMenu(player, _creature);
                 break;
