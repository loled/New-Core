...
void Spell::SpellDamageSchoolDmg (SpellEffIndex effIndex)
{
    ...
    if (unitTarget && unitTarget->isAlive())
    {
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            /*case 8092:
                ...
                if (m_caster->GetShapeshiftForm() == FORM_SHADOW)
                {
                   ...
                   // Orbe de las sombras
                    ...
                    {
                        float incr = 0.10f;
                        if(m_caster->HasAura(77486)) // Maestria
                        {
                            float mastery = m_caster->ToPlayer()->GetFloatValue(PLAYER_MASTERY);
                            incr += 0.0145*mastery;
                        }
                        damage += int32(damage * (incr * shadowOrb->GetStackAmount()));
			...
			if(m_spellInfo->Id == 8092)
			{
				...
				// Orbes de las sombras çBoTç // 17/03/2013 
				if(Aura* shadowOrb = m_caster->GetAura(77487))
				{
					float incr = 0.10f;
					if(m_caster->HasAura(77486)) // Cogiendo la maestria ... 
					{
						float mastery = m_caster->ToPlayer()->GetFloatValue(PLAYER_MASTERY);
						incr += 0.0145*mastery;
					}
                ...
                //Borrando explosion mental buf
                m_caster->RemoveAurasDueToSpell(87160);
                m_caster->RemoveAurasDueToSpell(81292);
			}
				
            // Pua mental arreglada ahora coge el daño de los orbes y lo suma çBoTç // 17/03/2013 
			...
			// Mejora para la pua mental (Forma de sombras)
	...
	case 35395: 
      ...
      break;
		...
		//Redefinición de efecto de daño en hechizos
		if (m_originalCaster && damage > 0 && apply_direct_bonus)
		damage = m_originalCaster->SpellDamageBonus(unitTarget, m_spellInfo, effIndex, (uint32) damage, SPELL_DIRECT_DAMAGE);	
		//Buff venganza BoT
        if(unitTarget->HasAura(93099) || unitTarget->HasAura(84839) || unitTarget->HasAura(93098))
        {
			int32 atkpwr = damage * 0.05f;
            if(AuraEffect* vng = unitTarget->GetAuraEffect(76691,0))
		...
		// 	Protección abisal fixed by BoT
		if(unitTarget->HasAura(30301) && unitTarget->HasAura(91713) || unitTarget->HasAura(30301) && unitTarget->HasAura(6229))
		{
			if (m_spellInfo->SchoolMask & SPELL_SCHOOL_MASK_HOLY)
				unitTarget->CastSpell(unitTarget, 54370, true);
			if (m_spellInfo->SchoolMask & SPELL_SCHOOL_MASK_FIRE)
				unitTarget->CastSpell(unitTarget, 54371, true);
			if (m_spellInfo->SchoolMask & SPELL_SCHOOL_MASK_NATURE)
				unitTarget->CastSpell(unitTarget, 54375, true);				
			if (m_spellInfo->SchoolMask & SPELL_SCHOOL_MASK_FROST)
				unitTarget->CastSpell(unitTarget, 54372, true);
			if (m_spellInfo->SchoolMask & SPELL_SCHOOL_MASK_SHADOW)
				unitTarget->CastSpell(unitTarget, 54374, true);
			if (m_spellInfo->SchoolMask & SPELL_SCHOOL_MASK_ARCANE)
				unitTarget->CastSpell(unitTarget, 54373, true);
		}		
		// Sagrario interno reducción de daño BoT
		if(unitTarget->HasAura(588))
        {
            if(unitTarget->HasAura(14747)) // Rango 1
                damage -= damage * 0.02f; // Reduce 2%

            if(unitTarget->HasAura(14770)) // Rango 2
                damage -= damage * 0.04f; // Reduce 4%

            if(unitTarget->HasAura(14771)) // Rango 3
                damage -= damage * 0.06f; // Reduce 6%
        }		
		m_damage += damage;
	}
}	
...
void Spell::EffectDummy (SpellEffIndex effIndex)
{
    ...
    case SPELLFAMILY_GENERIC:
    {
        switch (m_spellInfo->Id)
        {
				//REPARACION HABILIDADES DE LAS PET, empiezo con las del dk Profano
			/*case 47484: //Acurrurcarse
			{ 
				if (m_caster->HasAura(63560)) //Transformacion oscura
				{
					m_caster->CastSpell(m_caster, 91837, true);
					return;	
				}	
			...
			case 47481: //Roer
			{
				if (m_caster->HasAura(63560)) //Transformacion oscura
				{
					m_caster->CastSpell(m_caster, 91797, true);
					return;	
				}	
    ...
    case SPELLFAMILY_PRIEST:
    {
		...
		case 28133:
		{
			...
			{
				m_caster->CastSpell(unitTarget,64136,true);
			}
		}	
        case 73325:     	// Leap of faith
        {
			...
			//Cuerpo y alma + 60% speed BoT rango 2
			if(m_caster->HasAura(64129))
			{
				m_caster->CastSpell(unitTarget,65081,true);
			}
			//Cuerpo y alma + 60% speed BoT rango 1
    ...
    case SPELLFAMILY_MAGE:
    {
        /*if (m_spellInfo->Id == 11129)          //Combustion çBoTç // 17/03/2013 
        {           
            Unit::AuraEffectList const &mPeriodic = unitTarget->GetAuraEffectsByType(SPELL_AURA_PERIODIC_DAMAGE);
			int32 bp = 100;
            for (Unit::AuraEffectList::const_iterator i = mPeriodic.begin(); i != mPeriodic.end(); ++i)
    ...
    case SPELLFAMILY_DEATHKNIGHT:
        ...
        // Golpe letal reparado BoT
        if (m_spellInfo->SpellFamilyFlags[0] & SPELLFAMILYFLAG_DK_DEATH_STRIKE)
        {
           if (m_spellInfo->SpellFamilyFlags[0] & SPELLFAMILYFLAG_DK_DEATH_STRIKE)
            {
					int32 bp;
					if ((m_caster->CountPctFromMaxHealth(7)) > (20 * m_caster->GetDamageTakenInPastSecs(5) / 100) /*&& !m_caster->HasAura(50041)*/) //Añadida exepcion para el glifo golpe letal && ->>
						bp = m_caster->CountPctFromMaxHealth(7);
		//SIN TERMINAR  if ((m_caster->CountPctFromMaxHealth(7)) > (20 * m_caster->GetDamageTakenInPastSecs(5) / 100) && m_caster->HasAura(50041) && m_caster->HasAura(48266) || (m_caster->CountPctFromMaxHealth(7)) > (20 * m_caster->GetDamageTakenInPastSecs(5) / 100) && m_caster->HasAura(50041) && m_caster->HasAura(4))	 //Añadida exepcion para el glifo golpe letal && ->>
						/*bp = m_caster->CountPctFromMaxHealth(15);*/
					else
                ...
                // Escudo de sangre by BoT
                if (AuraEffect const* aurEff = m_caster->GetAuraEffect(50029, 1))
                {
                    // Presencia de sangre (Requerimiento)
                    if (m_caster->HasAura(48263))
                    {
                        // int32 shield = CalculatePctN(bp, 100) * float(m_caster->ToPlayer()->GetMasteryPoints() * 0.0625f);
						/*damage *= float(1.0f + (0.45f + (m_caster->ToPlayer()->GetMasteryPoints() * 0.056f)));*/
        ...
        // Espiral de la muerte, formula reajustada por BoT
    ...
    if (m_caster->ToPlayer() && m_caster->ToPlayer()->HasSkill(SKILL_ARCHAEOLOGY))
    {
        for (uint8 i = 1; i < 28; ++i)
        {
            ...
            if (rp->spellId == m_spellInfo->Id)
            {
                ...
                m_caster->ToPlayer()->ModifyCurrency(currencyId, -(reqFragments*100));
...
void Spell::EffectUntrainTalents (SpellEffIndex effIndex)
{
	...
	m_caster->RemoveAurasDueToSpell(76691);
}
...
void Spell::SpellDamageWeaponDmg (SpellEffIndex effIndex)
{
    ...
    case SPELLFAMILY_HUNTER:
    {
        ...
        case 56641:          // Steady Shot
        {
                    ...
                    shotMod = 0.021f;
                    if(Aura* fire = m_caster->GetAura(82925)) // Maestro tirador
                    {
                        if(fire->GetStackAmount() >= 4)
                        {
                            m_caster->CastSpell(m_caster,82926,true); // Disparo de punteria/Fuego!
...
void Spell::EffectScriptEffect (SpellEffIndex effIndex)
{
    ...
    {		
        ...
        {           
			...
			m_caster->CastCustomSpell(unitTarget,83853,&bp,&bp,NULL,true);
        }
		break;
    }
    case SPELLFAMILY_PRIEST:
...
void Spell::EffectActivateSpec (SpellEffIndex /*effIndex*/)
{
	...
	unitTarget->RemoveAurasDueToSpell(76691);  // Borrar venganza al cambiar la spe
}
