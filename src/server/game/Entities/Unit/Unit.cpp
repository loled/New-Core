...
uint32 Unit::DealDamage (Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss)
{
    ...
    else
    {
        ...
        if (duel_hasEnded)
        {
			...
			he->RemoveAurasDueToSpell(76691);
            he->DuelComplete(DUEL_WON);
        }
    }

    sLog->outStaticDebug("DealDamageEnd returned %d damage", damage);
	//Venganza de daño melee BoT
    if(pVictim->HasAura(93099) || pVictim->HasAura(84839) || pVictim->HasAura(93098))
    {
        int32 atkpwr = damage * 0.05f;
        
        if(AuraEffect* vng = pVictim->GetAuraEffect(76691,0)) //
                atkpwr += vng->GetAmount();
        if(atkpwr > int32(pVictim->CountPctFromMaxHealth(10)))
            atkpwr = int32(pVictim->CountPctFromMaxHealth(10));
        pVictim->RemoveAurasDueToSpell(76691);
        pVictim->CastCustomSpell(pVictim, 76691, &atkpwr, &atkpwr, NULL, true);
    }
    return damage;
}
...
bool Unit::HandleDummyAuraProc (Unit *pVictim, uint32 damage, AuraEffect* triggeredByAura, SpellEntry const * procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
    ...
    case SPELLFAMILY_MAGE:
    {
       ...
       // Buena racha + mejorada. BoT // 17/03/2012
		...
		// Ignicion, debuff reparado. BoT // 17/03/2012
		case 11119:
		case 11120:
		case 12846:
		{
			switch (dummySpell->Id)
			{
				case 11119: basepoints0 = int32(0.13f * damage);
				case 11120: basepoints0 = int32(0.26f * damage);
				case 12846: basepoints0 = int32(0.4f * damage);
			...
			int bp = damage * 0.40f;          //5% from damage
            CastCustomSpell(target, 12654, &bp, &bp, &bp, true, 0, 0, GetGUID());
			break;
        }
		case 12654:
		{
    ...
    case SPELLFAMILY_WARLOCK:
    {
		...
		// Burning Embers
		if (dummySpell->SpellIconID == 5116)
		{
			basepoints0 = CalculatePctN(int32(damage), triggerAmount) /7;
    ...
    case SPELLFAMILY_PRIEST:
    {
        ...
        case 81292:
        {
            ...
            // Contricion  by BoT - Reparado el rango 1 + exepcion para el -50% cuando la sanación es en uno mismo.
        case 14523: 
        {
			if (Player* caster = triggeredByAura->GetCaster()->ToPlayer())
				basepoints0 = int32(CalculatePctN(damage, triggerAmount) /4);
			else
				basepoints0 = int32(CalculatePctN(damage, triggerAmount) /2);
...
bool Unit::HandleAuraProc (Unit * pVictim, uint32 damage, Aura * triggeredByAura, SpellEntry const * procSpell, uint32 /*procFlag*/, uint32 procEx, uint32 cooldown, bool * handled)
{
    ...
    case SPELLFAMILY_MAGE:
    {
        ...
        /*case 11129:
        {
            *handled = true;
            ...
            return (procEx & PROC_EX_CRITICAL_HIT) ? true : false;         // ordinary chrages will be removed during crit chance computations.
        }*/
        // Empowered Fire
        case 31656:
        case 31657:
	...
	case SPELLFAMILY_DEATHKNIGHT:
        {
            // Scarlet Fever
            if(dummySpell->SpellIconID == 4891)
                if(pVictim->GetGUID() == this->GetGUID()) // Don't proc on self
                    return false;
            // Blood of the North
           /* // Reaping
...
bool Unit::HandleProcTriggerSpell (Unit *pVictim, uint32 damage, AuraEffect* triggeredByAura, SpellEntry const *procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    case 79684:
    {
        if (HasAura(44445) || HasAura(44446) || HasAura(44448))
            return false;
		break;	
    }
        // Cheat Death
