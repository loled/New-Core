...
void Player::RegenerateAll()
{
    ...
    if (m_regenTimerCount >= 2100)
    {
        ...
        m_regenTimerCount -= 1900;
    }

    if (m_focusRegenTimerCount >= 1100 && getClass() == CLASS_HUNTER)
    {
			Regenerate(POWER_FOCUS);
			m_focusRegenTimerCount -= 900;
    }

    if (m_holyPowerRegenTimerCount >= 10100 && getClass() == CLASS_PALADIN)
    {
        Regenerate(POWER_HOLY_POWER);
        m_holyPowerRegenTimerCount -= 9000;
    }
...
void Player::RegenerateHealth ()
{
    ...
    {
        addvalue = HealthIncreaseRate;          //2 secs: 0.75*2 :-) source: wowpedia.
        if (!isInCombat())
        {
		    // Ajustada regeneración de vida a Cataclysm --> BoT // 17/03/2013
			if (getLevel() < 15)
      ...
      if (!IsStandState())
        ...
        if (RACE_TROLL)
            addvalue *= 1.1f; // Regeneracion de los trolls + 10% çBoTç // 17/03/2013 
    }
...
uint32 Player::_GetCurrencyWeekCap (const CurrencyTypesEntry* currency) const
{
	...
	//DEFINIDO EL CAP DE LAS MONEDAS DE ARQUEOLOGIA
    case CURRENCY_TIPE_ARCHEAOLOGY_DWARF:
	{
	    uint32 cap = 100 * 200000;
	}	
...
void Player::SetVisibleItemSlot (uint8 slot, Item *pItem)
{
     if (pItem)
     {
        if (Transmogrification::GetFakeEntry(pItem)) // custom
...
bool Player::HasTradableItem(Item* item) const
{
    return std::find(m_itemSoulboundTradeable.begin(), m_itemSoulboundTradeable.end(), item) != m_itemSoulboundTradeable.end();
}
...
#define ARCHAEOLOGY_DIG_SITE_REWARD_AMMT_MAX		7
 ...
 void Player::SpawnArchaeologyScope()
 {
     ...
     } else
     {
        ...
        // Definido el número de monedas que recibes al excavar, adaptado a 4.0.6 BoT
         GenerateResearchProject(branchId, false, 0);
         ModifyCurrency(currencyId, urand(300, 400));
