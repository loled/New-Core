...
void ObjectMgr::LoadResearchSitesInfo()
{
     ...
     if (!result) 
     {
         sLog->outError(">> Loaded `research_sites`, table is empty!");
         return;
     }
     ...
     do 
     {
         ...
         if (!rs)
         {
             sLog->outError("ResearchSite %u defined in `research_sites` does not exist in DBC");
             continue;
         }
	...
	sLog->outError(">> Loaded %u Research Dig Sites info", count);
}
